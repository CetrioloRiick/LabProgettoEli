...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < N_events; ++i) {
    ...
    for (int j = 0; j < 100; ++j) {
      ...
      double type{0};
      type = gRandom->Rndm(); // numero random tra 0 e 1, ad ogni estrazione
                              // corrisponderà un tipo di particella
      if (type < 0.4) {
      ...
      double trasverse_impulse =
          std::sqrt(fPx * fPx + fPy * fPy); // impulso trasverso
